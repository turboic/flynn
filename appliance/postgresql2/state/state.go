...
func (p *Peer) handlePgInit(e PostgresEvent) {
	p.log.Info("postgres init", "at", "init_pg", "online", e.Online, "setup", e.Setup)
	if p.pgOnline != nil {
...
func (p *Peer) handleDiscoverdInit(e *DiscoverdEvent) {
	p.log.Info("discoverd init", "at", "init_discoverd", "peers", len(e.Peers), "state", e.State.State != nil)
...
func (p *Peer) handleDiscoverdPeers(e *DiscoverdEvent) {
	p.log.Info("discoverd peers", "at", "discoverd_peers", "peers", len(e.Peers))
	if p.clusterPeers == nil {
...
func (p *Peer) handleDiscoverdState(e *DiscoverdEvent) {
	...
	if p.decodeState(e) {
		...
		log.Info("already have this state", "at", "skip_eval")
	}
}
...
func (p *Peer) evalClusterState() {
	...
	if p.clusterState.Freeze != nil {
		log.Info("cluster frozen, not making any changes", "at", "frozen")
		return
	}
...
func (p *Peer) startInitialSetup() {
	...
	if err := p.putClusterState(); err != nil {
		log.Error("failed to create cluster state", "at", "create_state", "err", err)
	} else {
...
func (p *Peer) startUpdateAsyncs(newAsync []*discoverd.Instance) {
	...
	if err != nil {
		log.Error("failed to update cluster state", "err", err, "at", "update_state")
	} else {
		p.clusterState = p.updatingState
	}
	p.updatingState = nil
